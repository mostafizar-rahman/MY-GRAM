...
function Singup() {
    const { userRegister, userUpdateProfile, userLoginWithGoogle, loading } = useContext(ContextProvider)
        ...
        if (imageUrl) {
            ...
            userRegister(email, password)
                .then(result => {
                    ...
                    fetch('https://mygram-server.vercel.app/user', {
                        method: 'POST',
                        headers: {
    ...
    return (
        ...
        <div className="w-full max-w-sm mx-auto py-10  px-3">
            <h1 className="text-2xl font-bold text-center">Register</h1>
            <p className='text-red-600 capitalize'>{error}</p>
            <form onSubmit={handleSingupFormSubmit} className="space-y-6">
                ...
                <div className='pt-3 '>
                    <button type='submit' className='btn bg-red-700 w-full text-white rounded-md  relative min-h-[40px]'>{loading ?
